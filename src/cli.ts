...
import type { Device, DoorPolicy, TimePlanRequest, Weekday, CatRequest } from "./types.js";

const POLICIES: DoorPolicy[] = ["OPEN", "CLOSED", "OPEN_IN", "OPEN_OUT"];
...
function normalizePolicy(p: string): DoorPolicy {
  const u = String(p).toUpperCase().replace(/-/g, "_");
  if (!(POLICIES as string[]).includes(u)) {
...
function parseWeekdays(spec: string): Weekday[] {
...
function summarizeSettings(s: SettingsLike | undefined): string {
  if (!s) return "";
  const parts: string[] = [];
